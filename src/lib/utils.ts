...
type ContentCollectionName = "blog" | "project";
...
export const getOptionalCollection = async <
  TCollection extends ContentCollectionName,
>(
  collection: TCollection,
  filter?: (entry: CollectionEntry<TCollection>) => boolean,
  ...
  try {
    ...
    if (
      ...
      error.message.includes(`The collection "${collection}" does not exist or is empty`)
    ) {
      return [];
    }
    throw error;
